...
// Load primitives from the JSON format used in the Python file.
std::vector<std::unique_ptr<Primitive>> loadPrimitivesFromJson(const std::string &path);

// Create a normalized grid as vector of 2D coords [0,1]x[0,1]
std::vector<Eigen::Vector2d> createNormalizedGrid(int H, int W);

// Render primitives to an RGBA image (floating point 0..1)
cv::Mat renderSoft(const std::vector<std::unique_ptr<Primitive>> &prims, int H, int W, double softness=150.0);

// Save primitives to a simple text file (similar to Python save_parameters)
void saveParameters(const std::string &path, const std::vector<std::unique_ptr<Primitive>> &prims, int H, int W);
